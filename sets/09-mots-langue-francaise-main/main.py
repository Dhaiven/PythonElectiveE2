...
def ensemble_mots(filename)->set:
...
def mots_de_n_lettres(mots: set, n):
    ...
    return set([mot for mot in mots if len(mot) == n])
...
def mots_avec(mots, s):
    ...
    return set([mot for mot in mots if s in mot])
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    for mot in mots:
        ...
        # finit par un suffixe
        if not any(mot.endswith(s) for s in lstop):
            continue

        # contient un élément de lmid qui n'est ni au début ni à la fin
        mid_ok = False
        for mid in lmid:
            pos = mot.find(mid)
            if pos > 0 and pos + len(mid) < len(mot):  # ni début ni fin
                mid_ok = True
                break
